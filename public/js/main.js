document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const response = await fetch('/api/all-words?limit=1');
    ...
    if (data.pagination && typeof data.pagination.totalWords === 'number') {
      wordCountDisplay.textContent = data.pagination.totalWords;
    } else {
      wordCountDisplay.textContent = '0';
    }

  } catch (error) {
    console.error('Не вдалося отримати статистику:', error);
    wordCountDisplay.innerHTML = '<span class="text-danger" style="font-size: 1rem;">Помилка</span>';
  }
